...
class Vote extends React.Component<IVoteProps, IVoteState> {
  ...
  doUpdate = async (id) => {
    this.api.query.democracy.referendumInfoOf(id, (referendum: Option<ReferendumInfo>) => {
      ...
      if (referendumInfo !== undefined) {
        ...
        const proposal = referendumInfo.proposal as unknown as Extrinsic
        const { meta, methodName, sectionName } = new GenericCall(proposal.callIndex)
  ...
  renderProposal () {
    const loadAye = this.state.loading ? <div style={{ marginLeft: '6px', marginTop: '1px' }}><Icon loading={true} name='asterisk'/></div> : 'Aye'
    const loadNay = this.state.loading ? <div style={{ marginLeft: '5px', marginTop: '1px' }}><Icon loading={true} name='asterisk'/></div> : 'Nay'
    const { chartColorAye, chartColorNay, backgroundColor } = colorSchemes[this.props.settings.color]
    return (
      <ContentContainer>
        <AccountDropdown/>
